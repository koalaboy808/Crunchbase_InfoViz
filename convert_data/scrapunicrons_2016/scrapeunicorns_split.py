...
def main():
	...
	soup = getSoup(url1)
	getResults(soup, namestoday)

	soup = getSoup(url2)
	getResults(soup, namesexited)

	soup = getSoup(url3)
	getResults(soup, namesemerging)


	f = open("unicornstoday.csv", 'w')
	for item in namestoday:
		f.write(item+"\n")


	f = open("unicornsexited.csv", 'w')
	for item in namesexited:
		f.write(item+"\n")


	f = open("unicornsemerging.csv", 'w')
	for item in namesemerging:
		f.write(item+"\n")
